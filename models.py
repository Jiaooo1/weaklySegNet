...
class ConvUpBlock(nn.Module):
    ...
    def forward(self, x, x_skip):
        x = self.up(x)
        #print('x.shape:', x.shape) # x.shape: torch.Size([1, 256, 14, 14])
        H_diff = x.shape[2] - x_skip.shape[2]
        W_diff = x.shape[3] - x_skip.shape[3]
        #print('H_diff, W_diff:', H_diff, W_diff) # H_diff, W_diff: 0 0
...
class ResUNet34(nn.Module):
    ...
    def forward(self, x):
        #print('x.shape:', x.shape) #x.shape: torch.Size([4, 3, 224, 224])
        ...
        print('s4.shape:', s4.shape)  # s4.shape: torch.Size([4, 256, 14, 14])
        print('s2.shape:', s2.shape)  # s2.shape: torch.Size([4, 64, 56, 56])
        print('s1.shape:', s1.shape)  # s1.shape: torch.Size([4, 64, 112, 112])
        '''
        x1 = self.u5(x, s4)
        x1 = self.u6(x1, s3)
        x1 = self.u7(x1, s2)
        x1 = self.u8(x1, s1)
        #print('x1.shape1:', x1.shape)  # x1.shape1: torch.Size([4, 64, 112, 112])
        x1 = self.seg(x1)
        #print('x1.shape:', x1.shape)  # x1.shape: torch.Size([4, 1, 224, 224])
        return x1
...
#新加的
class boundary_fusion_module(nn.Module):
    def __init__(self, in_channel, out_channel, dropout_rate=0.0, dilation=1, device = None, dtype = None, ** kwargs, ):
        factory_kwargs = {"device": device, "dtype": dtype}
        super().__init__()
        self.up0 = nn.ConvTranspose2d(in_channel * 2, out_channel, 16, stride=16)
        self.up1 = nn.ConvTranspose2d(in_channel, out_channel, 8, stride=8)
        self.up2 = nn.ConvTranspose2d(in_channel//2, out_channel, 4, stride=4) #卷积核大小为2
        self.up3 = nn.ConvTranspose2d(in_channel//4, out_channel, 2, stride=2)

        self.attention = CBAM(in_channel = out_channel)

        self.conv = nn.Conv2d(  # 尺寸不变
            in_channels=320,
    ...
    def forward(self, e0, e1, e2, e3, e4):
        #print('e0.shape:', e0.shape) # e0.shape: torch.Size([1, 512, 7, 7])
        up_e0 = self.up0(e0)
        up_e1 = self.up1(e1)
        up_e2 = self.up2(e2)
        up_e3 = self.up3(e3)
        #print('e0.shape', e0.shape)
        #print('up_e0.shape', up_e0.shape)
        #print('e1.shape', e1.shape) # e1.shape torch.Size([1, 256, 14, 14])
        #print('up_e1.shape', up_e1.shape)  # up_e1.shape torch.Size([1, 64, 112, 112])
        #print('up_e2.shape', up_e2.shape)
        #print('up_e3.shape', up_e3.shape)  # up_e3.shape torch.Size([1, 64, 112, 112])
        #print('e4.shape', e4.shape)  # e4.shape torch.Size([1, 64, 112, 112])
        e = torch.cat([up_e0, up_e1, up_e2, up_e3, e4], 1)
        #print('e.shape', e.shape)  # e.shape torch.Size([1, 320, 112, 112])
        e = self.conv(e)
        e = self.attention(e)
...
class AffResUNet34(nn.Module):
    def __init__(self, pretrained=False, post=False):
        ...
        # 边界融合模块  新加的
        self.BFM = boundary_fusion_module(l[3], l[0], dropout_rate=0.1)
    ...
    def forward(self, x):
        # print('x.shape0:', x.shape)  # x.shape0: torch.Size([1, 3, 224, 224])
        # refer https://github.com/pytorch/vision/blob/master/torchvision/models/resnet.py
        ...
        #print('x.shape:', x.shape)  # x.shape: torch.Size([1, 512, 7, 7])
        ...
        #print('s4.shape0:', s4.shape) # s4.shape0: torch.Size([1, 256, 14, 14])
        # 新加的，边界信息感知模块(融合每一层解码器的信息，使用亲和力图计算损失)
        e1 = self.u5_edge(x, s4)  # 256
        e2 = self.u6_edge(e1, s3)  # 128
        e3 = self.u7_edge(e2, s2)  # 64
        e4 = self.u8_edge(e3, s1)  # 64
        e = self.BFM(x, e1, e2, e3, e4)
        edge = self.edge(e)
...
class AffinityLoss(AffResUNet34):  # AffinityLoss 继承自 AffResUNet34 类
    ...
    def to_affinity(self, edge):
        ...
        for i in range(self.n_path_lengths): #path of length 1,2,3...
            ...
            # 在 dist 张量的第三维度上应用最大池化，在路径长度维度上获取每个路径的最大值
            # 从 1 减去池化结果，生成亲和性度量。这样，大的池化值（表示距离较远或差异较大的边）将被转换为较低的亲和性值。
            aff_list.append(aff)
    ...
    def forward(self, x):
        seg, edge = super().forward(x) # 56 56 # 执行父类AffResUNet34的forward方法
        return seg, edge
