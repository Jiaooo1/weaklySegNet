...
class CoarseDataset(data.Dataset):
    def __init__(self, img_dir, vor_dir, clu_dir, po_dir, aug=False):
        img_list = []
        ...
        transform = {
            'train': {
                ...
                'vertical_flip': True,  # 比伪标签优化的baseline多一个垂直翻转
                'random_affine': 0.3,
                'random_rotation': 90,  # 随机旋转
                'random_crop': 224,
                'label_encoding': 3,  # 标签编码，标签种类为3
                'to_tensor': 1
            },
            'val': {
                'random_crop': 224,
    ...
    def __getitem__(self, idx):
        ...
        clu = Image.open(self.clu_dir + '/' + img_name + '_label_cluster.png').convert('RGB')
        #print(self.vor_dir + img_name + '_label_vor.png')
        po = Image.open(self.po_dir + '/' + img_name + '_label_po.png').convert('RGB')

        img, vor, clu, po= self.transform((img,vor,clu,po))
        return img, vor, clu, po

class CoarseDataset1(data.Dataset):
    def __init__(self, img_dir, vor_dir, mixed_dir, aug=False):
        img_list = []
        img_names = os.listdir(img_dir)
        for img_name in img_names:
            if not is_image_file(img_name):
    ...
    def __getitem__(self, idx):
        ...
        img = Image.open(self.img_dir + '/' + img_name + '.png').convert('RGB')
        vor = Image.open(self.vor_dir + '/' + img_name + '_label_vor.png').convert('RGB')
        mixed = Image.open(self.mixed_dir + '/' + img_name + '_label_mixed.png').convert('RGB')

        img, vor, mixed= self.transform((img,vor,mixed))
        return img, vor, mixed

class CoarseDataset2(data.Dataset):
...
class AffinityFromInstance(data.Dataset):
    def __init__(self, img_dir, aff_dir, vor_dir, mixed_dir, indices_from, indices_to, aug=False):
        super().__init__()
        img_list = []
    ...
    def __getitem__(self, idx):
        ...
        new_label = np.ones((mixed.shape[0], mixed.shape[1]), dtype=np.uint8) * 2
        new_label[mixed[:, :, 0] > 255 * 0.3] = 0
        new_label[mixed[:, :, 1] > 255 * 0.5] = 1
        new_label[(img[:, :, 0] == 0) * (img[:, :, 1] == 0) * (img[:, :, 2] == 0)] = 0
        mixed = new_label.astype(np.uint8)
        ...
        mixed = mixed[sh:sh+224, sw:sw+224]
        ...
        mixed = torch.tensor(mixed.copy()).unsqueeze(0)

        return img, bg_pos, fg_pos, neg, vor, mixed


class AffinityFromInstance1(data.Dataset):
...
# 新加的
class AffinityFromInstance2(data.Dataset):
    def __init__(self, img_dir, aff_dir, vor_dir, clu_dir, indices_from, indices_to, aug=False):
        super().__init__()
        img_list = []
        img_name_list = os.listdir(img_dir)
    ...
    def __getitem__(self, idx):
        ...
        # to tensor
        img = torch.tensor(img.copy().transpose(2, 0, 1) / 255)
        #bg_pos, fg_pos, neg = self.extract_aff_lab_func(aff.copy()) # 原来
        aff = torch.from_numpy(aff.copy()).unsqueeze(0)
        vor = torch.tensor(vor.copy()).unsqueeze(0)
        clu = torch.tensor(clu.copy()).unsqueeze(0)
        #print('bg_pos:', bg_pos.shape)  # bg_pos: torch.Size([96, 45570])
        #print('aff.shape:', aff.shape)  # aff.shape: torch.Size([1, 224, 224])
        #print('vor.shape:', vor.shape)  # vor.shape: torch.Size([1, 224, 224])

        return img, aff, vor, clu
