...
from models import _resnet, BasicBlock, dilated_conv
...
class PatchEmbed2D(nn.Module):
    ...
    """
    def __init__(self, patch_size=4, in_chans=3, embed_dim=256, norm_layer=None, **kwargs):
        super().__init__()
...
class Final_PatchExpand2D(nn.Module):
    ...
    def forward(self, x):
        B, H, W, C = x.shape
        print('x.shape:', x.shape)  # x.shape: torch.Size([1, 1, 1, 512])
        x = self.expand(x)

        x = rearrange(x, 'b h w (p1 p2 c)-> b (h p1) (w p2) c', p1=self.dim_scale, p2=self.dim_scale, c=C)
        x= self.norm(x)
        print('x.shape:', x.shape)  # x.shape: torch.Size([1, 4, 4, 128])

        return x


class general_conv2d(nn.Module):
    def __init__(self, in_ch, out_ch, k_size=3, stride=1, padding=1, pad_type='reflect', is_training=True, device=None,
                 dtype=None, **kwargs, ):
...
class SS2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        xz = self.in_proj(x)  # nn.Linear
        x, z = xz.chunk(2, dim=-1)

        x = x.permute(0, 3, 1, 2).contiguous()
        x = self.act(self.conv2d(x)) # (b, d, h, w)  #nn.Conv2d(3,1)
        y1, y2, y3, y4 = self.forward_core(x)
        assert y1.dtype == torch.float32
        y = y1 + y2 + y3 + y4
        y = torch.transpose(y, dim0=1, dim1=2).contiguous().view(B, H, W, -1)
        y = self.out_norm(y)  # nn.LinearNorm
        ...
        if self.dropout is not None:
            out = self.dropout(out)  # nn.Dropout
        return out
...
class VSSM(nn.Module):
    ...
    '''
    # 原来
    def __init__(self, patch_size=4, in_chans=3, num_classes=1, depths=[6],
                 dims=[512],  d_state=16, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, patch_norm=True,
                 use_checkpoint=False, with_uncertainty=False, **kwargs):
        ...
        self.patch_embed = PatchEmbed2D(patch_size=patch_size, in_chans=dims[0], embed_dim=self.embed_dim,
        ...
        self.resnet = resnet34(**kwargs)  # 新加的
        # up conv
        l = [64, 64, 128, 256, 512]
        ...
        if self.with_uncertainty:
            self.seg = nn.ConvTranspose2d(l[0], 2, 2, stride=2)
            self.seg2 = nn.ConvTranspose2d(l[0], 2, 2, stride=2)
        else:
            self.seg = nn.ConvTranspose2d(l[0], 1, 2, stride=2)
            self.seg2 = nn.ConvTranspose2d(l[0], 1, 2, stride=2)
    ...
    def forward(self, x):
        ...
        x_out = self.u5(x, s4)
        x_out = self.u6(x_out, s3)
        x_out = self.u7(x_out, s2)
        x_out = self.u8(x_out, s1)
        x1 = self.seg(x_out)
