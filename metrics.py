...
def compute_metrics(pred, gt, names):
    ...
    #print('pred.dtype:', pred.dtype)  # pred.dtype: int32
    #print('gt.dtype:', gt.dtype)  # gt.dtype: int32
    gt_labeled = label(gt)
    pred_labeled = label(pred)

    gt_binary = gt_labeled.copy()
    pred_binary = pred_labeled.copy()
    non_zero_count = np.count_nonzero(gt_binary)
    non_zero_count1 = np.count_nonzero(pred_binary)
    #print('gt非0个数：', non_zero_count)  # 标签为连通区域以及严格的每个细胞核属于一个连通区域（单独一个id）时，这个值一样
    #print('pred非0个数：', non_zero_count1)
    gt_binary[gt_binary > 0] = 1
    pred_binary[pred_binary > 0] = 1
    # 这两行代码会将 pred 和 gt 中所有非零值都转换为 1，其余为 0
    gt_binary, pred_binary = gt_binary.flatten(), pred_binary.flatten()
    # gt 和 pred 被处理为二进制图像（gt_binary 和 pred_binary），并且它们被转换为 1D 数组（flatten()）
