...
# 新加的
def split_forward_uncertainty(model, input, size, overlap, outchannel=2, with_uncertainty=False):
    '''
...
def split_forward_single_uncertainty(model, input, size, overlap):
    '''
    split the input image for forward passes
    '''
    ...
    output2 = torch.zeros((input.size(0), 2, h, w))
    for i in range(0, h-overlap, size-overlap):
        r_end = i + size if i + size < h else h
        ind1_s = i + overlap // 2 if i > 0 else 0
        ind1_e = i + size - overlap // 2 if i + size < h else h
