...
def main():
    #dataset = 'MO'  # LC: Lung Cancer, MO: MultiOrgan #原来
    dataset = 'MoNuSeg'
    #dataset = 'CPM'
    data_dir = './{:s}'.format(dataset)
    img_dir = './{:s}/images'.format(dataset)
    label_instance_dir = './{:s}/labels_instance_differentid_fillPoly'.format(dataset)
    #label_instance_dir = './{:s}/labels_instance_npy_differentid'.format(dataset)
    label_point_dir = 'D:/pycharm/weaklySegNet/data/{:s}/labels_point'.format(dataset)
    label_vor_dir = 'D:/pycharm/weaklySegNet/data/{:s}/labels_voronoi'.format(dataset)
    label_cluster_dir = 'D:/pycharm/weaklySegNet/data/{:s}/labels_cluster'.format(dataset)
    label_po_dir = 'D:/pycharm/weaklySegNet/data/{:s}/labels_po'.format(dataset)
    label_mixed_dir = 'D:/pycharm/weaklySegNet/data/{:s}/labels_mixed'.format(dataset)
    patch_folder = 'D:/pycharm/weaklySegNet/data/{:s}/patches'.format(dataset)
    train_data_dir = 'D:/pycharm/weaklySegNet/data_for_train/{:s}'.format(dataset)


    with open('{:s}/train_val_test_MO.json'.format(data_dir), 'r') as file:
        data_list = json.load(file)
        train_list = data_list['train']
        val_list = data_list['val'] #新加的
    ...
    create_point_label_from_instance(label_instance_dir, label_point_dir, combined_list)
    create_point_label_from_instance(label_instance_dir, label_point_dir, val_list)

    # ------ create Voronoi label from point label
    create_Voronoi_label(label_point_dir, label_vor_dir, combined_list)
    create_Voronoi_label(label_point_dir, label_vor_dir, val_list)

    # ------ create cluster label from point label and image
    create_cluster_label(img_dir, label_point_dir, label_vor_dir, label_cluster_dir, combined_list)
    create_cluster_label(img_dir, label_point_dir, label_vor_dir, label_cluster_dir, val_list)
    ...
    split_patches(label_po_dir, '{:s}/labels_po'.format(patch_folder), 'label_po')
    split_patches(label_mixed_dir, '{:s}/labels_mixed'.format(patch_folder), 'label_mixed')

    split_patches(label_vor_dir, '', 'label_vor')
    split_patches(label_cluster_dir, '', 'label_cluster')
...
def create_h_component_image(img_dir, save_dir):
    ...
    for image_name in image_list:
        ...
        imageio.imsave('{:s}/{:s}_label_Hcomponent.png'.format(save_dir, image_name[:-4]), image_h)
...
def create_point_label_from_instance(data_dir, save_dir, train_list):
    ...
    for image_name in image_list:
        ...
        id_max = np.max(image)
        print('id_max:',id_max)
        label_point = np.zeros((h, w), dtype=np.int32)
        ...
        #imageio.imsave('{:s}/{:s}_label_point.png'.format(save_dir, name), label_point.astype(np.uint8))
        np.save("D:/pycharm/weaklySegNet/data/MoNuSeg/labels_point_npy/" + name + ".npy", label_point)
...
def create_Voronoi_label(data_dir, save_dir, train_list):
    ...
    for img_name in sorted(img_list):
        ...
        if '{:s}'.format(name[:-12]) not in train_list:
            continue
...
def create_cluster_label(data_dir, label_point_dir, label_vor_dir, save_dir, train_list):
    ...
    for img_name in sorted(img_list):
        name = img_name.split('.')[0]
        # if img_name not in train_list:  #原来
        if name not in train_list:
            continue
        ...
        imageio.imsave('{:s}/{:s}_label_cluster.png'.format(save_dir, name), refined_label)
...
def split_patches(data_dir, save_dir, post_fix=None):
    ...
    for image_name in image_list:
        ...
        num_patches_h = (h - patch_size) / (patch_size - h_overlap) + 1
...
def organize_data_for_training(data_dir, train_data_dir):
    ...
    create_folder('{:s}/labels_po'.format(train_data_dir))
    ...
    create_folder('{:s}/labels_po/train'.format(train_data_dir))
    ...
    with open('{:s}/train_val_test_MO.json'.format(data_dir), 'r') as file:
    #with open('{:s}/train_val_test_CPM.json'.format(data_dir), 'r') as file:
        data_list = json.load(file)
    ...
    for img_name in train_list:
        ...
        for file in glob.glob('{:s}/patches/images/{:s}*'.format(data_dir, name)):
            str='{:s}/patches/images/{:s}*'.format(data_dir, name)
...
def compute_mean_std(data_dir, train_data_dir):
    ...
    for file_name in train_list:
        img_name = '{:s}/images/{:s}.tif'.format(data_dir, file_name)
        #img_name = '{:s}/images/{:s}.png'.format(data_dir, file_name)
    ...
    #print(total_square_sum / num_pixel - mean_values ** 2)  # [-30874.51543972 -14928.85209541 -25943.94243224]
    std_values = np.sqrt(mean_values ** 2 - total_square_sum / num_pixel)
